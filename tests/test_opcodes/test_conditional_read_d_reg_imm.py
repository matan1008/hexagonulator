...
def test_conditional_read_d_reg_imm(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\xc0\x41')
...
def test_conditional_read_d_reg_imm_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\xc0\x41')
    add_memory(hexagon, b'\x00\x11\x22\x33\x44\x55\x66\x77', 0x40000020)
    hexagon.registers.general[0] = 0x40000018
    hexagon.cycle()
...
def test_conditional_read_d_reg_imm_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x04\x21\xc0\xc0\x41')
    add_memory(hexagon, b'\x00\x11\x22\x33\x44\x55\x66\x77', 0x40000020)
