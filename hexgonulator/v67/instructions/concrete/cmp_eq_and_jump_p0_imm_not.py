...
class CmpEqAndJumpP0ImmNot(CmpEqAndJump):
    @classmethod
    def from_int(cls, instr, apply_extension=None):
        ...
        return cls(instr, r=r9, s=s, pu=0, imm=imm, sense=False)

    def __repr__(self):
        return f'p{self.pu}=cmp.eq(R{self.s},#{self.imm}); if (!p{self.pu}.new) jump:nt PC + #{self.r}'
