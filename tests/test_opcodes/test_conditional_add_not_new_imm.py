...
def test_conditional_add_not_new_imm(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\xc1\xff\x80\x74')
...
def test_conditional_add_not_new_imm_predicate_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\xc1\xff\x80\x74')
    hexagon.registers.general[0] = 4
...
def test_conditional_add_not_new_imm_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x0f\xc1\xff\x80\x74')
