...
def test_conditional_read_d_reg_imm_not_new(hexagon):
    mem = RAM(8)
    ...
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\xc0\x47')
...
def test_conditional_read_d_reg_imm_not_new_false(hexagon):
    mem = RAM(8)
    mem.write(0, 8, b'\x00\x11\x22\x33\x44\x55\x66\x77')
    hexagon.memory.controllers.append(MemoryController(mem, start=0x40000020, end=0x40000028))
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\xc0\x47')
...
def test_conditional_read_d_reg_imm_not_new_apply_extension(hexagon):
    mem = RAM(8)
    mem.write(0, 8, b'\x00\x11\x22\x33\x44\x55\x66\x77')
    hexagon.memory.controllers.append(MemoryController(mem, start=0x40000020, end=0x40000028))
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x04\x21\xc0\xc0\x47')
