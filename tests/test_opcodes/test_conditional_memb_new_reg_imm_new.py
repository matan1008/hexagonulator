...
def test_conditional_memb_new_reg_imm_new(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x03\x40\x62\x70\x00\xe2\xa0\x42')
...
def test_conditional_memb_new_reg_imm_new_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x03\x40\x62\x70\x00\xe2\xa0\x42')
    mem = add_memory(hexagon, b'\x00\x00\x00\x00', 0x40000020)
    hexagon.registers.general[0] = 0x40000000
...
def test_conditional_memb_new_reg_imm_new_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 12, b'\x03\x40\x62\x70\x00\x40\x00\x04\x00\xc2\xa0\x42')
    mem = add_memory(hexagon, b'\x00\x00\x00\x00', 0x40000020)
