...
class ConditionalMembNewRegRegOffNotNew(ConditionalMembNewIndirectRegisterOffset):
    @classmethod
    def from_int(cls, instr, apply_extension=None):
        ...
        return cls(instr, t=t, pv=pv, s=s, u=u, shift=imm, sense=False, dot_new=True)

    def __repr__(self):
        return f'if (!P{self.pv}.new) memb(R{self.s}+R{self.u}<<#{self.shift})=N{self.t}.new'
