...
def test_conditional_read_uh_reg_imm_new(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\x60\x43')
...
def test_conditional_read_uh_reg_imm_new_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\x60\x43')
    add_memory(hexagon, b'\x00\xf0\x00\x00', 0x40000020)
...
def test_conditional_read_uh_reg_imm_new_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x04\x21\xc0\x60\x43')
    add_memory(hexagon, b'\x00\xf0\x00\x00', 0x40000020)
