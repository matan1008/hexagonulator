...
class ConditionalReadDRegRegOffNotNew(ConditionalReadDIndirectRegisterOffset):
    @classmethod
    def from_int(cls, instr, apply_extension=None):
        ...
        return cls(instr, s=s, t=t, d=d, shift=imm, pv=pv, sense=False, dot_new=True)

    def __repr__(self):
        return f'if (!P{self.pv}.new) R{self.d + 1}:{self.d}=memd(R{self.s}+R{self.t}<<#{self.shift})'
