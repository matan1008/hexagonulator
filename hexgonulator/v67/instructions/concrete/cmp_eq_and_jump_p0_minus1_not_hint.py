...
class CmpEqAndJumpP0Minus1NotHint(CmpEqAndJump):
    @classmethod
    def from_int(cls, instr, apply_extension=None):
        ...
        return cls(instr, r=r9, s=s, pu=0, imm=-1, sense=False, hint=True)

    def __repr__(self):
        return f'p{self.pu}=cmp.eq(R{self.s},#-1); if (!p{self.pu}.new) jump:t PC + #{self.r}'
