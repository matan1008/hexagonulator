...
def test_memb_new_m_circ(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x03\x40\x62\x70\x02\xc2\xa0\xa9')
...
def test_memb_new_m_circ_overflow(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x03\x40\x62\x70\x02\xc2\xa0\xa9')
    mem = add_memory(hexagon, b'\x00\x00\x00\x00', 0x40000020)
    hexagon.registers.m0.i = 1
...
def test_memb_new_m_circ_underflow(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x03\x40\x62\x70\x02\xc2\xa0\xa9')
    mem = add_memory(hexagon, b'\x00\x00\x00\x00', 0x40000020)
