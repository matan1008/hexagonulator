...
from ..abstract.memb_fifo_absolute_register_offset import MembFifoAbsoluteRegisterOffset


class MembFifoImmRegOff(MembFifoAbsoluteRegisterOffset):
    @classmethod
    ...
    def __repr__(self):
        return f'R{self.y + 1}:{self.y}=memb_fifo(R{self.t}<<#{self.shift}+#{self.imm})'
