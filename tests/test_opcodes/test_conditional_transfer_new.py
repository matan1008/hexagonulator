...
def test_conditional_transfer_new(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\xc0\xff\x0f\x7e')
    hexagon.xunits = HookedXunits(hexagon, lambda: set_predicate(hexagon, 1))
    hexagon.cycle()
    assert hexagon.registers.general[0] == 0xfffffffe


def test_conditional_transfer_new_predicate_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\xc0\xff\x0f\x7e')
...
def test_conditional_transfer_new_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x0f\xc0\xff\x0f\x7e')
    hexagon.xunits = HookedXunits(hexagon, lambda: set_predicate(hexagon, 1))
    hexagon.cycle()
