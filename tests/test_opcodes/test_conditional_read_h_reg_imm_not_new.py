...
def test_conditional_read_h_reg_imm_not_new(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\x40\x47')
...
def test_conditional_read_h_reg_imm_not_new_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x21\xc0\x40\x47')
    add_memory(hexagon, b'\x00\xf0\x00\x00', 0x40000020)
    hexagon.registers.general[0] = 0x4000001e
...
def test_conditional_read_h_reg_imm_not_new_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x04\x21\xc0\x40\x47')
    add_memory(hexagon, b'\x00\xf0\x00\x00', 0x40000020)
