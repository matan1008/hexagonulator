...
def test_conditional_read_ub_imm_new(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x80\xf0\x30\x9f')
...
def test_conditional_read_ub_imm_new_false(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 4, b'\x80\xf0\x30\x9f')
    add_memory(hexagon, b'\xf0\x00\x00\x00', 0x20)
...
def test_conditional_read_ub_imm_new_apply_extension(hexagon):
    hexagon.memory.controllers[0].mem.write(0, 8, b'\x00\x40\x00\x04\x80\xf0\x30\x9f')
