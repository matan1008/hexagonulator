...
class CmpGtuAndJumpP0ImmNotHint(CmpGtuAndJump):
    @classmethod
    def from_int(cls, instr, apply_extension=None):
        ...
        return cls(instr, r=r9, s=s, pu=0, imm=imm, sense=False, hint=True)

    def __repr__(self):
        return f'p{self.pu}=cmp.gtu(R{self.s},#{self.imm}); if (!p{self.pu}.new) jump:t PC + #{self.r}'
